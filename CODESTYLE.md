...
1. Immediately after the opening brace and immediately before the closing brace of a type, extension, initializer, function, or multi-line closure. These blank lines bookend the body: they are applied symmetrically, so a body has a blank line at both the top and the bottom, or at neither. A body consisting of a single line omits both.
...
A body is bookended with a blank line at the top and the bottom. The same rule applies to the type or extension that contains it:
...
A body consisting of a single line omits both blank lines:
...
The bookending blank lines are never applied to only one end:
...
A blank comment line — a line containing only `///` — separates the last parameter from the `Returns` tag. A `Throws` tag is separated from the tag that precedes it in the same way.
